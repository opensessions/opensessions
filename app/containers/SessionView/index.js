...
export default class SessionView extends React.Component { // eslint-disable-line react/prefer-stateless-function
  ...
  getActions(session) {
    ...
    if (this.canAct('touch')) actions.push(<Button key="touch" onClick={() => apiModel.action('session', session.uuid, 'touch').then(result => actionNext(result, this.context.notify, this.context.router))} className={publishStyles.previewButton}>Touch</Button>);
    ...
    if (this.canAct('updateMySchedule')) actions.push(<Button key="updateMySchedule" onClick={() => confirm(`Currently ${session.sortedSchedule.filter(s => s.hasOccurred).length} of your session's dates are in the past. This will automatically update your schedule so all dates are in the future. Are you sure you want to continue?`) && apiModel.action('session', session.uuid, 'updateMySchedule').then(result => actionNext(result, this.context.notify, this.context.router, () => this.fetchData()))} className={publishStyles.previewButton}>Auto-update schedule</Button>);
